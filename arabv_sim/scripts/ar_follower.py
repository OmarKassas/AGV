...
roll = pitch = yaw = V = Vx =Vy = Vth = 0.0
...
class ARFollower():
    def __init__(self):
        ...
        self.goal_x = rospy.get_param("~goal_x", 0.01)
        ...
        self.y_threshold = rospy.get_param("~y_threshold", 0.02)
        ...
        self.cmd_vel_pub = rospy.Publisher('/arabv/base_controller/cmd_vel', Twist, queue_size=5)
    ...
    def set_cmd_vel(self, msg):
        global roll, pitch, yaw, Vx, Vy, Vth, V
        ...
        if abs(yaw) > 0.03:
            ...
            # Vth = copysign(max(self.min_angular_speed,
            #             min(self.max_angular_speed, abs(speed))), speed)
        ...
        #     if speed < 0:
        #         speed *= 1.5
        #     V = 0.5*sqrt((target_offset_x)**2 + (target_offset_y)**2)
        #     # self.move_cmd.linear.x = copysign(min(self.max_linear_speed, max(self.min_linear_speed, abs(speed))), speed)
        #     print("target_offset_x = ", target_offset_x)
        #     print("Center is near!")
        # else:
        #     V = 0
        ...
        #     self.move_cmd.angular.z = -1*copysign(max(self.min_angular_speed,
        #                                 min(self.max_angular_speed, abs(speed))), speed)
        #     print("AR_Tag is near!")
        # else:
        #     self.move_cmd.angular.z = 0.0
        #     print("stop!")
        ...
        #     if speed < 0:
        #         speed *= 1.5
        #     self.move_cmd.linear.x = -1*copysign(min(self.max_linear_speed, max(self.min_linear_speed, abs(speed))), speed)
        #     print("Center is near!")
        # else:
        #     self.move_cmd.linear.x = 0.0
        #     print("STOP!")
